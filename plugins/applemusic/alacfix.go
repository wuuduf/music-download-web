...
//
// Ported from zhaarey/apple-music-downloader (utils/alacfix/alacfix.go). The
// original lived in its own `alacfix` package; here it is folded into the
// `applemusic` package, and every generic top-level identifier carries an
// `alac` prefix so it cannot collide with names already used elsewhere in the
// package. The bitstream parsing logic itself is unchanged.
//
...
var errAlacEOF = errors.New("bit reader EOF")

func newAlacBitReader(buf []byte) *alacBitReader {
	return &alacBitReader{buf: buf, nbits: len(buf) * 8}
}

func (b *alacBitReader) left() int { return b.nbits - b.pos }

func (b *alacBitReader) read(n int) (uint32, error) {
	if n == 0 {
		return 0, nil
	}
	if b.pos+n > b.nbits {
...
func (b *alacBitReader) show(n int) (uint32, error) {
	save := b.pos
	v, err := b.read(n)
	b.pos = save
	return v, err
...
func (b *alacBitReader) skip(n int) error {
	if b.pos+n > b.nbits {
		return errAlacEOF
	}
	b.pos += n
...
func (b *alacBitReader) readSigned(n int) (int32, error) {
	v, err := b.read(n)
	if err != nil {
		return 0, err
	}
...
func (b *alacBitReader) unary09() (uint32, error) {
	cnt := uint32(0)
	for cnt < 9 {
		v, err := b.read(1)
		if err != nil {
...
func alacDecodeScalar(br *alacBitReader, k int, bps int) (uint32, error) {
	x, err := br.unary09()
	if err != nil {
		return 0, err
	}
...
func alacRiceDecompress(br *alacBitReader, nbSamples int, bps int, rhmEff uint32, p *alacParams) error {
	...
	for i := 0; i < nbSamples; {
		...
		k := alacAvLog2((history >> 9) + 3)
		if k > limit {
			k = limit
		}
		x, err := alacDecodeScalar(br, k, bps)
		if err != nil {
			return err
		}
		x = x + signMod
		...
		if history < 128 && (i+1) < nbSamples {
			k2 := 7 - alacAvLog2(history) + int((history+16)>>6)
			if k2 > limit {
				k2 = limit
			}
			blockSize, err := alacDecodeScalar(br, k2, 16)
			if err != nil {
				return err
			}
			if blockSize > 0 {
...
// alacScanOneElement consumes one element from br. Returns (channels_used,
// is_end_tag, error). If an unsupported element tag is hit, returns an error.
func alacScanOneElement(br *alacBitReader, p *alacParams) (int, bool, error) {
	elem, err := br.read(3)
	if err != nil {
		return 0, false, err
	}
	...
	if isCompressed {
		...
		for c := 0; c < channels; c++ {
			...
			if err := alacRiceDecompress(br, int(outputSamples), bps, rhmEff, p); err != nil {
				return 0, false, err
			}
		}
	} else {
...
// alacFindBodyEndBit returns the bit position right after the last non-END
// element body. -1 means parse failure.
func alacFindBodyEndBit(packet []byte, p *alacParams) int {
	br := newAlacBitReader(packet)
	chUsed := 0
	lastEnd := -1
	for br.left() >= 3 {
		nCh, isEnd, err := alacScanOneElement(br, p)
		if err != nil {
			return -1
		}
		if isEnd {
...
// alacFindChild returns the first direct child atom of the given type within [start, end).
func alacFindChild(buf []byte, start, end int, typ string) (alacAtom, bool) {
	p := start
	for p < end-8 {
		...
		if atomType == typ {
			return alacAtom{typ: atomType, hdrOff: p, bodyOff: p + hdr, endOff: p + size}, true
		}
		p += size
	}
	return alacAtom{}, false
...
// alacFindAllChildren returns every direct child of the given type within [start, end).
func alacFindAllChildren(buf []byte, start, end int, typ string) []alacAtom {
	var out []alacAtom
	p := start
	for p < end-8 {
		size := int(binary.BigEndian.Uint32(buf[p : p+4]))
		...
		if atomType == typ {
			out = append(out, alacAtom{typ: atomType, hdrOff: p, bodyOff: p + hdr, endOff: p + size})
		}
		p += size
	}
	return out
...
func extractAlacConfig(data []byte, sampleEntry alacAtom) (alacParams, error) {
	// Sample entry has a fixed 28-byte audio header before any child atoms.
	childStart := sampleEntry.bodyOff + 28
	if cfg, ok := alacFindChild(data, childStart, sampleEntry.endOff, "alac"); ok {
	...
	if wave, ok := alacFindChild(data, childStart, sampleEntry.endOff, "wave"); ok {
		if cfg, ok := alacFindChild(data, wave.bodyOff, wave.endOff, "alac"); ok {
			if cfg.endOff-cfg.bodyOff < 28 {
				return alacParams{}, errors.New("alac config atom too small")
			}
			return parseAlacMagicCookie(data[cfg.bodyOff : cfg.bodyOff+28])
...
func alacReadPacketLocations(data []byte, stbl alacAtom) ([]alacPacketLoc, error) {
	stsz, ok := alacFindChild(data, stbl.bodyOff, stbl.endOff, "stsz")
	if !ok {
		return nil, errors.New("stsz missing")
	}
	stsc, ok := alacFindChild(data, stbl.bodyOff, stbl.endOff, "stsc")
	if !ok {
		return nil, errors.New("stsc missing")
	}
	stco, ok := alacFindChild(data, stbl.bodyOff, stbl.endOff, "stco")
	is64 := false
	if !ok {
		stco, ok = alacFindChild(data, stbl.bodyOff, stbl.endOff, "co64")
		if !ok {
	...
	for c, choff := range chunkOff {
		...
		for k := 0; k < spc && sampleIdx < len(sizes); k++ {
			...
			locs = append(locs, alacPacketLoc{offset: cur, size: sz})
			cur += int64(sz)
			sampleIdx++
		}
...
func findAlacTracks(data []byte) ([]alacTrackData, error) {
	...
	for _, trak := range alacFindAllChildren(data, moov.bodyOff, moov.endOff, "trak") {
		var trackID uint32
		if tkhd, ok := alacFindChild(data, trak.bodyOff, trak.endOff, "tkhd"); ok {
			b := tkhd.bodyOff
			version := data[b]
		...
		mdia, ok := alacFindChild(data, trak.bodyOff, trak.endOff, "mdia")
		if !ok {
			continue
		}
		// Require handler_type == 'soun'.
		hdlr, ok := alacFindChild(data, mdia.bodyOff, mdia.endOff, "hdlr")
		if !ok {
			continue
		}
		hb := hdlr.bodyOff
		...
		minf, ok := alacFindChild(data, mdia.bodyOff, mdia.endOff, "minf")
		if !ok {
			continue
		}
		stbl, ok := alacFindChild(data, minf.bodyOff, minf.endOff, "stbl")
		if !ok {
			continue
		}
		stsd, ok := alacFindChild(data, stbl.bodyOff, stbl.endOff, "stsd")
		if !ok {
			continue
		}
		...
		locs, err := alacReadPacketLocations(data, stbl)
		if err != nil {
			return nil, fmt.Errorf("track %d: %w", trackID, err)
		}
		tracks = append(tracks, alacTrackData{trackID: trackID, params: params, locs: locs})
	}
	return tracks, nil
}
...
func alacPatchInPlace(data []byte, off int64, size int, bodyEndBit int) bool {
	totalBits := size * 8
